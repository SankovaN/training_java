package training.addressbook.generators;
    ...
    private static List<ContactData> generateContacts(int count) {
        ...
        for (int i = 0; i < count; i++) {
            contacts.add(new ContactData().withName(String.format("test %s", i))
                    .withName(String.format("firstname %s", i)).withLastname(String.format("lastname %s", i))
                    .withEmail(String.format("nadya_sankova@mail.ru", i)).withAddress(String.format("address %s", i))
                    .withPhone(String.format("12345", i)));
        }
        return contacts;
    }
}
